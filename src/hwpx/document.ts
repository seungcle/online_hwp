...
function buildMatchers(prefix: string): TagMatcher[] {
  const p = `${prefix}:`
  return [
...
export function parseSection(bytes: Uint8Array, index: number, name: string): SectionModel {
  ...
  const matchers = buildMatchers(prefix)
...
export function buildDocumentModel(sections: SectionModel[]): DocumentModel {
  ...
  for (const section of sections) {
    ...
    stats.tableCount += countTables(section.blocks)
  }
  return { sections, stats }
}

function countTables(blocks: readonly Block[]): number {
  let total = 0
  for (const block of blocks) {
    if (block.kind !== 'table') continue
    total += 1
    for (const row of block.rows) {
      for (const cell of row) total += countTables(cell.blocks)
    }
  }
  return total
}
